...
- (FIRCollectionReference *)collectionWithPath:(NSString *)collectionPath {
    ...
    CollectionReference child = _documentReference.GetCollectionReference(
            MakeString(collectionPath));
    return [[FIRCollectionReference alloc] initWithReference:std::move(child)];
}

- (void)setData:(NSDictionary

<NSString *, id> *)documentData {
        ...
        void OnEvent(StatusOr<DocumentSnapshot> maybe_snapshot) override {
            if (maybe_snapshot.ok()) {
                FIRDocumentSnapshot *result =
                        [[FIRDocumentSnapshot alloc] initWithSnapshot:std::move(
