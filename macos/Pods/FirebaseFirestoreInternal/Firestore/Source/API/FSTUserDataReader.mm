...
- (ParsedSetData)parsedMergeData:(id)input fieldMask:(nullable NSArray
...
- (Message <google_firestore_v1_Value>)parseDictionary:(NSDictionary
...
:(ParseContext &&)context {
    ...
    if (dict.count == 0) {
        ...
        result->map_value.fields = nanopb::MakeArray<google_firestore_v1_MapValue_FieldsEntry>(
                count);
        ...
        switch (cType[0]) {
                ...
                // because when compiled for LP64, unsigned long is 64 bits and could overflow int64_t.
            {
                ...
                if (extended > LLONG_MAX) {
                    ThrowInvalidArgument("NSNumber (%s) is too large%s",
...
- (Message <google_firestore_v1_Value>)encodeReference:(const DatabaseId &)databaseId
    ...
    std::string referenceName = ResourcePath({"projects", databaseId.project_id(), "databases",
                                              databaseId.database_id(), "documents",
                                              key.ToString()})
            .CanonicalString();
