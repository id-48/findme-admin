...
<FIRAggregateField *> *)aggregateFields {
    if (self = [super init]) {
        ...
        _aggregateQuery =
                absl::make_unique<AggregateQuery>(
...
- (BOOL)isEqual:(nullable id)other {
    ...
    return [_query isEqual:otherQuery->_query] &&
           *_aggregateQuery == *(otherQuery->_aggregateQuery);
}
...
- (void)aggregationWithSource:(FIRAggregateSource)source
    ...
    _aggregateQuery->GetAggregate([self, completion](const StatusOr <ObjectValue> &result) {
        if (result.ok()) {
            completion(
                    [[FIRAggregateQuerySnapshot alloc] initWithObject:result.ValueOrDie() query:self],
                    nil);
        } else {
