...
using UserUpdateBlock = id
_Nullable (^)(FIRTransaction *, NSError **);
...
- (instancetype)initWithDatabaseID:(model::DatabaseId)databaseID
       ...
       appCheckCredentialsProvider:
          ...
          firebaseMetadataProvider:
                  ...
                  instanceRegistry:(nullable id

<FSTFirestoreInstanceRegistry>)registry {
    if (self = [super init]) {
        _firestore = std::make_shared<Firestore>(
                std::move(databaseID), std::move(persistenceKey),
                std::move(authCredentialsProvider),
...
+ (instancetype)firestoreForApp:(FIRApp *)app database:(NSString *)database {
    if (!app) {
        ThrowInvalidArgument(
                "FirebaseApp instance may not be nil. Use FirebaseApp.app() if you'd like "
...
- (FIRCollectionReference *)collectionWithPath:(NSString *)collectionPath {
    ...
    if ([collectionPath containsString:@"//"]) {
        ThrowInvalidArgument("Invalid path (%s). Paths must not contain // in them.",
                             collectionPath);
    }
...
- (FIRQuery *)collectionGroupWithID:(NSString *)collectionID {
    ...
    if ([collectionID containsString:@"/"]) {
        ThrowInvalidArgument(
                "Invalid collection ID (%s). Collection IDs must not contain / in them.",
                collectionID);
    }
...
- (void)runTransactionWithBlock:(id _Nullable
...
error))completion {
    ...
    dispatch_once(&onceToken, ^{
        transactionDispatchQueue = dispatch_queue_create(
...
- (void)enableNetworkWithCompletion:(nullable void (^)(NSError *_Nullable error)
...
- (void)clearPersistenceWithCompletion:(nullable void (^)(NSError *_Nullable error)

)completion {
    _firestore->ClearPersistence(MakeCallback(completion));
}

- (void)waitForPendingWritesWithCompletion:(void (^)(NSError *_Nullable error)

)completion {
    _firestore->WaitForPendingWrites(MakeCallback(completion));
}

- (void)terminateWithCompletion:(nullable void (^)(NSError *_Nullable error)
...
- (FIRLoadBundleTask *)loadBundle:(nonnull NSData

*)bundleData {
    auto stream = absl::make_unique<ByteStreamApple>(
            [[NSInputStream alloc] initWithData:bundleData]);
...
- (FIRLoadBundleTask *)loadBundleStream:(NSInputStream *)bundleStream
                             completion:
                                     (nullable void (^)(FIRLoadBundleTaskProgress *_Nullable
                                                        progress,
                                                        NSError *_Nullable error)
...
- (void)getQueryNamed:(NSString *)name completion:(void (^)(FIRQuery *_Nullable query))completion {
    ...
    auto callback = [completion, firestore](core::Query query, bool found) {
        ...
        if (found) {
            FIRQuery *firQuery = [[FIRQuery alloc] initWithQuery:std::move(
...
- (nullable FIRPersistentCacheIndexManager
...
- (void)terminateInternalWithCompletion:(nullable void (^)(NSError *_Nullable error)
