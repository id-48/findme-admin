...
- (FIRFirestore *)firestoreForDatabase:(NSString *)database {
    ...
    @synchronized (self.instances) {
        ...
        if (!firestore) {
            ...
            id <FIRAppCheckInterop> app_check = FIR_COMPONENT(FIRAppCheckInterop,
                                                              self.app.container);
            ...
            auto firebaseMetadataProvider = absl::make_unique<FirebaseMetadataProviderApple>(
                    self.app);
            ...
            firestore = [[FIRFirestore alloc] initWithDatabaseID:std::move(databaseID)
                                     ...
                                     appCheckCredentialsProvider:std::move(
...
- (void)appWillBeDeleted:(__unused FIRApp
...
<FIRComponent *> *)componentsToRegister {
    ...
    creationBlock:
    ^id
    _Nullable(FIRComponentContainer * container, BOOL * isCacheable)
    {
