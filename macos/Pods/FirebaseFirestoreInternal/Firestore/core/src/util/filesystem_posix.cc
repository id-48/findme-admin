            ...
            Status Filesystem::RemoveFile(const Path &path) {
                if (::unlink(path.c_str())) {
                    if (errno != ENOENT) {
                        return Status::FromErrno(
                                errno,
                                StringFormat("Could not delete file %s", path.ToUtf8String()));
                    }
                }
                return Status::OK();
            }
                ...
                PosixDirectoryIterator::PosixDirectoryIterator(const util::Path &path)
                    ...
                    if (!dir_) {
                        status_ = Status::FromErrno(
                                errno,
                                StringFormat("Could not open directory %s",
                                             parent_.ToUtf8String()));
                        return;
                    }
                    Advance();
                }
                ...
                void PosixDirectoryIterator::Advance() {
                    ...
                    if (!entry_) {
                        if (errno != 0) {
                            status_ = Status::FromErrno(
                                    errno,
                                    StringFormat("Could not read %s", parent_.ToUtf8String()));
                        }
                    } else if (status_.ok()) {
