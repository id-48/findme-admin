...
// If a non-empty list occurs during parsing, the parsing failed.
    ...
    public:
        ...
        void PushField(absl::string_view ext)
        ...
        void AddError(absl::string_view error)

        GPR_ATTRIBUTE_NOINLINE;

        // Returns true if the current field has errors.
        bool FieldHasErrors() const
...
// Specializations of AutoLoader for vectors.
        template<typename T>
        class AutoLoader<std::vector < T>>

        final : public LoadVector {
        private:
        ~

        AutoLoader() = default;

        void *EmplaceBack(void *dst) const
    ...
    const LoaderInterface *ElementLoader() const
...
// doesn't work.
template<>
class AutoLoader<std::vector < bool>>
...
template<typename T>
class AutoLoader<std::map < std::string, T>>

final : public LoadMap {
private:

void *Insert(const std::string &name, void *dst) const
...
template<typename T>
class AutoLoader<absl::optional < T>>

final : public LoadOptional {
public:

void *Emplace(void *dst) const

final {
return &static_cast<absl::optional <T>*>(dst)->
...
if (
LoadObject(json, args, elements_
.
