...
// Given F(A,B,C,...), what's the return type?
    ...
    struct ResultOfT<F(Args...),
            absl::void_t < decltype(std::declval < RemoveCVRef < F >> ()(
                    std::declval<Args>()...))>> {
    using T =
    decltype(std::declval<RemoveCVRef < F>>
    ()(
...
private:
    GPR_NO_UNIQUE_ADDRESS F
    f_;
    GPR_NO_UNIQUE_ADDRESS Arg
    arg_;
};
...
PromiseFactoryImpl(F
&& f,
A &&arg
) {
return Curried<RemoveCVRef < F>, A>(
...
public:
    ...
    decltype (PromiseFactoryImpl(std::move(f_)));
