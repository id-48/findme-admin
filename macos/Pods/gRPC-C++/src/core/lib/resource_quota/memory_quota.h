...
// then that reclamation is complete and we may continue the reclamation loop.
    ...
    public:
        ...
        template<typename F>
        GRPC_MUST_USE_RESULT OrphanablePtr<Handle>
        Insert(F
        reclaimer) {
        ...
        GRPC_MUST_USE_RESULT NextPromise

        Next() { return NextPromise(this); }
...
// be) but to be eventually accurate.
    ...
    private:
        ...
        kInitialSize = std::numeric_limits<intptr_t>::max();
...
// underlying resource quota.
    ...
    private:
        ...
        absl::optional <size_t> TryReserve(MemoryRequest request)
        ...
        void MaybeRegisterReclaimer()

        ABSL_LOCKS_EXCLUDED(reclaimer_mu_);
        ...
        Mutex reclaimer_mu_;
        bool shutdown_
        ABSL_GUARDED_BY(reclaimer_mu_) = false;
