                ...
                if (allocator_ != nullptr) {
                    ...
                    allocator_state =
                            new(grpc::g_core_codegen_interface->grpc_call_arena_alloc(
                                    call,
                                    sizeof(DefaultMessageHolder < RequestType, ResponseType >)))
                ...
                void Finish(grpc::Status s) override {
                    ...
                    finish_tag_.Set(
                            ...
                            [this](bool) {
                                this->MaybeDone(
                                        reactor_.load(
                                                std::memory_order_relaxed)->InternalInlineable());
                            },
        ...
        private:
            ...
            *,
            ...
            private:
                ...
                std::atomic<ServerReadReactor < RequestType>*>
                reactor_;
                // callbacks_outstanding_ follows a refcount pattern
                std::atomic<intptr_t> callbacks_outstanding_{
        ...
        private:
            ...
            *,
            ...
            private:
                ...
                std::function<void()> call_requester_;
                // The memory ordering of reactor_ follows ServerCallbackUnaryImpl.
                std::atomic<ServerWriteReactor < ResponseType>*>
                reactor_;
                // callbacks_outstanding_ follows a refcount pattern
                std::atomic<intptr_t> callbacks_outstanding_{
                        3};  // reserve for OnStarted, Finish, and CompletionOp
        ...
        private:
            ...
            *)>
            ...
            private:
                ...
                std::function<void()> call_requester_;
                // The memory ordering of reactor_ follows ServerCallbackUnaryImpl.
                std::atomic<ServerBidiReactor < RequestType, ResponseType>*>
                reactor_;
                // callbacks_outstanding_ follows a refcount pattern
                std::atomic<intptr_t> callbacks_outstanding_{
                        3};  // reserve for OnStarted, Finish, and CompletionOp
