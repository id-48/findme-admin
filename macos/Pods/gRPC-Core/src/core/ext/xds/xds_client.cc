...
{
    public:
    explicit RetryableCall(WeakRefCountedPtr<ChannelState>
    chand);
...
private:
    ...
    private:
        ...
        ABSL_GUARDED_BY(&XdsClient::mu_) = true;
        absl::optional <EventEngine::TaskHandle> timer_handle_
        ABSL_GUARDED_BY(&XdsClient::mu_);
    };
    ...
    bool send_message_pending_
    ABSL_GUARDED_BY(&XdsClient::mu_) = false;
...
// it is shutting down.
xds_client_->xds_server_channel_map_.
erase(server_);
ads_calld_.
...
ProcessAdsResponseFields(AdsResponseFields fields) {
    if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_client_trace)) {
        gpr_log(
                ...
                "version=%s, nonce=%s, num_resources=%"
        PRIuPTR,
...
void XdsClient::ChannelState::LrsCallState::OnRecvMessage(
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_xds_client_trace)) {
        gpr_log(
                GPR_INFO,
                "[xds_client %p] xds server %s: LRS response received, %"
        PRIuPTR
        " cluster names, send_all_clusters=%d, load_report_interval=%"
        PRId64
        "ms",
                xds_client(), chand()->server_.server_uri.c_str(),
        ...
        for (const auto &name: new_cluster_names) {
            gpr_log(GPR_INFO, "[xds_client %p] cluster_name %"
            PRIuPTR
            ": %s",
