...
// RLS LB policy.
        ...
        private:
            ...
            public:
                ...
                void MaybeFinishUpdate()

                ABSL_LOCKS_EXCLUDED(&RlsLb::mu_);
            ...
            public:
                ...
                private:
                    ...
                    private:
                        ...
                        bool armed_
                        ABSL_GUARDED_BY(&RlsLb::mu_) = true;
                    ...
                    bool is_shutdown_
                    ABSL_GUARDED_BY(&RlsLb::mu_) = false;
                    ...
                    ABSL_GUARDED_BY(&RlsLb::mu_);
                    Timestamp backoff_time_
                    ABSL_GUARDED_BY(&RlsLb::mu_) =
                            Timestamp::InfPast();
                    Timestamp backoff_expiration_time_
                    ABSL_GUARDED_BY(&RlsLb::mu_) =
                            Timestamp::InfPast();
                    ...
                    ABSL_GUARDED_BY(&RlsLb::mu_);
                    Timestamp data_expiration_time_
                    ABSL_GUARDED_BY(&RlsLb::mu_) =
                            Timestamp::InfPast();
                    Timestamp stale_time_
                    ABSL_GUARDED_BY(&RlsLb::mu_) = Timestamp::InfPast();

                    Timestamp min_expiration_time_
                    ABSL_GUARDED_BY(&RlsLb::mu_);
            ...
            private:
                ...
                private:
                    ...
                    std::deque <Timestamp> requests_
                    ABSL_GUARDED_BY(&RlsLb::mu_);

                    // Logged timestamps of failures.
                    std::deque <Timestamp> failures_
                    ABSL_GUARDED_BY(&RlsLb::mu_);
                };
            ...
            OrphanablePtr <RlsChannel> rls_channel_
            ABSL_GUARDED_BY(mu_);
...
//
        ...
        void RlsLb::Cache::Resize(size_t bytes) {
            if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_rls_trace)) {
                gpr_log(GPR_INFO, "[rlslb %p] resizing cache to %"
                PRIuPTR
                " bytes",
                        lb_policy_, bytes);
            }
        ...
        void RlsLb::Cache::MaybeShrinkSize(size_t bytes) {
            ...
            if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_rls_trace)) {
                gpr_log(GPR_INFO,
                        "[rlslb %p] LRU pass complete: desired size=%"
                PRIuPTR
                " size=%"
                PRIuPTR,
                ...
                if (names_array->empty()) {
                    ...
                    for (const Json &name_json: *names_array) {
                        ...
                        if (!GRPC_ERROR_IS_NONE(child_error)) {
                            ...
                            if (!inserted) {
                                error_list.push_back(GRPC_ERROR_CREATE_FROM_CPP_STRING(
                                        absl::StrCat("field:names error:duplicate entry for ",
                                                     name)));
                            }
                        }
                    }
