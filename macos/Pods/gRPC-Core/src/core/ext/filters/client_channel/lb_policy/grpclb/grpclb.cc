...
//
        ...
        std::string GrpcLb::Serverlist::AsText() const {
            ...
            for (size_t i = 0; i < serverlist_.size(); ++i) {
                ...
                entries.push_back(absl::StrFormat("  %"
                PRIuPTR
                ": %s token=%s\n", i,
            ...
            if (!GrpcLbResponseParse(response_slice, arena.ptr(), &response) ||
                ...
                switch (response.type) {
                        ...
                        if (response.client_stats_report_interval != Duration::Zero()) {
                            ...
                            if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_glb_trace)) {
                                gpr_log(GPR_INFO,
                                        ...
                                        "message; client load reporting interval = %"
                                PRId64
                                " milliseconds",
                                        grpclb_policy(), this,
                    ...
                    case response.SERVERLIST: {
                        ...
                        if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_glb_trace)) {
                            gpr_log(GPR_INFO,
                                    "[grpclb %p] lb_calld=%p: Serverlist with %"
                            PRIuPTR
                            " servers received:\n%s",
                                    grpclb_policy(), this,
        ...
        void GrpcLb::StartBalancerCallRetryTimerLocked() {
            ...
            if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_glb_trace)) {
                ...
                if (timeout > Duration::Zero()) {
                    gpr_log(GPR_INFO, "[grpclb %p] ... retry_timer_active in %"
                    PRId64
                    "ms.",
                            this, timeout.millis());
                } else {
