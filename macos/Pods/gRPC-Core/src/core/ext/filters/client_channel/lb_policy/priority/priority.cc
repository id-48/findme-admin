...
namespace grpc_core {
    ...
    namespace {
        ...
        kPriority = "priority_experimental";
...
// have switched to a higher-priority child).
        constexpr Duration
        kChildRetentionInterval = Duration::Minutes(15);
...
// before starting to attempt the next priority.  Overridable via channel arg.
        constexpr Duration
        kDefaultChildFailoverTimeout = Duration::Seconds(10);
...
//
        ...
        PriorityLb::ChildPriority::FailoverTimer::FailoverTimer(
            ...
            if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_priority_trace)) {
                gpr_log(
                        GPR_INFO,
                        "[priority_lb %p] child %s (%p): starting failover timer for %"
                PRId64
                "ms",
