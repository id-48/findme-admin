...
// easily see the functions that are shared, and helps reduce code bloat here.
    ...
    struct SimpleIntBasedMetadata : public SimpleIntBasedMetadataBase<Int> {
        ...
        invalid_value() { return kInvalidValue; }
...
return
NameLookup<void, Traits...>::Lookup(key, op
);
}
};
...
public:
    ...
    template<typename Trait>
    GPR_ATTRIBUTE_NOINLINE ParsedMetadata<Container>
    Found(Trait
    trait) {
...
public:
    ...
    GPR_ATTRIBUTE_NOINLINE absl::optional<absl::string_view>
    NotFound(
            absl::string_view
    key) {
...
GPR_NO_UNIQUE_ADDRESS StorageType
value;
};
...
template<typename Derived, typename... Traits>
MetadataMap<Derived, Traits...>::MetadataMap(MetadataMap &&other)
