...
GPR_GLOBAL_CONFIG_DEFINE_BOOL(grpc_experimental_smooth_memory_presure,
false,
...
GPR_GLOBAL_CONFIG_DEFINE_INT32(
        grpc_experimental_max_quota_buffer_size,
1024 * 1024,
"Maximum size for one memory allocators buffer size against a quota");
GPR_GLOBAL_CONFIG_DEFINE_INT32(
        grpc_experimental_resource_quota_set_point,
95,
        ...
        while (free > 0) {
            ...
            if (free_bytes_.compare_exchange_weak(free, new_free,
                ...
                if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace)) {
                    gpr_log(GPR_INFO, "[%p|%s] Early return %"
                    PRIdPTR
                    " bytes", this,
                            name_.c_str(), ret);
                }
...
//
    ...
    void BasicMemoryQuota::Start() {
        ...
        auto reclamation_loop = Loop(Seq(
                ...
                [self](
        std::tuple<const char *, RefCountedPtr < ReclaimerQueue::Handle>>
        arg) {
    ...
    void BasicMemoryQuota::SetSize(size_t
    new_size) {
