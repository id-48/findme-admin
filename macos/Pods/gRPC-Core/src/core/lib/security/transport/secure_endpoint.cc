...
namespace {
    struct secure_endpoint {
        ...
        ABSL_GUARDED_BY(read_mu);
        grpc_slice write_staging_buffer
        ABSL_GUARDED_BY(write_mu);
...
ABSL_EXCLUSIVE_LOCKS_REQUIRED(ep
->read_mu) {
grpc_slice_buffer_add_indexed(ep
->read_buffer, ep->read_staging_buffer);
ep->
read_staging_buffer =
...
->write_mu) {
grpc_slice_buffer_add_indexed(&ep
->output_buffer, ep->write_staging_buffer);
ep->
write_staging_buffer =
ep->memory_owner.MakeSlice(grpc_core::MemoryRequest(STAGING_BUFFER_SIZE));
*
