...
memmove(sb
->base_slices, sb->slices, sb->count * sizeof(grpc_slice));
sb->
slices = sb->base_slices;
} else {
...
sb->
base_slices = static_cast<grpc_slice *>(
        gpr_malloc(new_capacity * sizeof(grpc_slice)));
memcpy(sb
->base_slices, sb->inlined, slice_count * sizeof(grpc_slice));
} else {
sb->
base_slices = static_cast<grpc_slice *>(
...
sb->
slices = sb->base_slices + slice_offset;
}
}
...
uint8_t *grpc_slice_buffer_tiny_add(grpc_slice_buffer * sb, size_t
n) {
grpc_slice *back;
uint8_t *out;
...
return back->data.inlined.
bytes;
}

size_t grpc_slice_buffer_add_indexed(grpc_slice_buffer * sb, grpc_slice
s) {
size_t out = sb->count;
maybe_embiggen(sb);
sb->slices[out] =
...
void grpc_slice_buffer_add(grpc_slice_buffer * sb, grpc_slice
s) {
size_t n = sb->count;
...
refcount &&
        back
->data.inlined.length<GRPC_SLICE_INLINED_SIZE) {
if (s.data.inlined.length + back->data.inlined.length <=
GRPC_SLICE_INLINED_SIZE) {
memcpy(back
->data.inlined.bytes + back->data.inlined.length,
...
back->data.inlined.
length = GRPC_SLICE_INLINED_SIZE;
maybe_embiggen(sb);
back = &sb->slices[n];
sb->
count = n + 1;
...
memcpy(back
->data.inlined.bytes, s.data.inlined.bytes + cp1,
s.data.inlined.length - cp1);
}
sb->length += s.data.inlined.
length;
return; /* early out */
}
}
...
void grpc_slice_buffer_addn(grpc_slice_buffer * sb, grpc_slice * s, size_t
n) {
size_t i;
for (
i = 0;
...
static void slice_buffer_move_first_maybe_ref(grpc_slice_buffer * src, size_t
n,
grpc_slice_buffer *dst
) {
GPR_ASSERT(src
...
->length == output_len);
GPR_ASSERT(src
->length == new_input_len);
GPR_ASSERT(src
->count > 0);
}

void grpc_slice_buffer_move_first(grpc_slice_buffer * src, size_t
n,
grpc_slice_buffer *dst
) {
...
void grpc_slice_buffer_move_first_no_ref(grpc_slice_buffer * src, size_t
n,
grpc_slice_buffer *dst
) {
...
void grpc_slice_buffer_move_first_into_buffer(grpc_slice_buffer * src, size_t
n,
void *dst
) {
...
void grpc_slice_buffer_copy_first_into_buffer(grpc_slice_buffer * src, size_t
n,
void *dst
) {
...
void grpc_slice_buffer_trim_end(grpc_slice_buffer * sb, size_t
n,
...
if (slice_len > n) {
sb->slices[idx] =
grpc_slice_split_head(&slice, slice_len
- n);
if (garbage) {
...
void grpc_slice_buffer_sub_first(grpc_slice_buffer * sb, size_t
begin,
size_t end
) {
// TODO(soheil): Introduce a ptr version for sub.
sb->length -=
GRPC_SLICE_LENGTH(sb
->slices[0]);
sb->slices[0] =
grpc_slice_sub_no_ref(sb
->slices[0], begin, end);
sb->length += end -
begin;
}
