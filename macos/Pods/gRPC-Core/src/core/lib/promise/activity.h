...
// An atomic variant of Waker - this type is non-copyable and non-movable.
    ...
    public:
        ...
        AtomicWaker(AtomicWaker &&other)

        noexcept = delete;

        AtomicWaker &operator=(AtomicWaker &&other)
...
// called with absl::CancelledError().
    ...
    private:
        ...
        static GPR_THREAD_LOCAL(Activity
        *)
        g_current_activity_;
    };
...
// accidental casting.
    ...
    protected:
        ...
        ABSL_LOCK_RETURNED(mu_) {return &mu_;}
    ...
    private:
        ...
        ActionDuringRun action_during_run_
        ABSL_GUARDED_BY(mu_) =
...
// when we move away from C++11 and have more powerful template features.
    ...
    private:
        ...
        absl::optional <ResultType> RunStep()
        ...
        // poll the promise.
        absl::optional <ResultType> StepLoop()
        ...
        GPR_NO_UNIQUE_ADDRESS WakeupScheduler
        wakeup_scheduler_;
        // Callback on completion of the promise.
        GPR_NO_UNIQUE_ADDRESS OnDone
        on_done_;
        // Has execution completed?
        GPR_NO_UNIQUE_ADDRESS bool done_

        ABSL_GUARDED_BY (mu()) = false;

        // Is there a wakeup scheduled?
        GPR_NO_UNIQUE_ADDRESS std::atomic<bool>
        ...
        union PromiseHolder {
            ...
            GPR_NO_UNIQUE_ADDRESS Promise
            promise;
        };

        GPR_NO_UNIQUE_ADDRESS PromiseHolder

        promise_holder_ ABSL_GUARDED_BY(mu());
    };
