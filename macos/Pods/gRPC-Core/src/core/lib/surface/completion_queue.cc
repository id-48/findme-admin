...
// NOTE: Only one event will ever be cached.
    GPR_THREAD_LOCAL(grpc_cq_completion
    *)
    g_cached_event;
    GPR_THREAD_LOCAL(grpc_completion_queue
    *)
    g_cached_cq;
...
static void cq_end_op_for_next(
        grpc_completion_queue * cq, void * tag, grpc_error_handle
error,
...
static grpc_event cq_next(grpc_completion_queue * cq, gpr_timespec
deadline,
void *reserved
);
...
#ifndef NDEBUG
static void cq_check_tag(grpc_completion_queue * cq, void * tag, bool
lock_cq) {
int found = 0;
if (lock_cq) {
gpr_mu_lock(cq
...
void *done_arg,
        grpc_cq_completion
* storage, bool /*internal*/) {
GPR_TIMER_SCOPE("cq_end_op_for_next", 0);

if (
GRPC_TRACE_FLAG_ENABLED(grpc_api_trace)
||
(
...
gpr_log(GPR_INFO,
"Operation failed: tag=%p, error=%s", tag,
errmsg.
...
done_arg = done_arg;
storage->
next = static_cast<uintptr_t>(is_success);

cq_check_tag(cq, tag,
true); /* Used in debug builds only */
...
if (!
GRPC_ERROR_IS_NONE(kick_error)
) {
gpr_log(GPR_ERROR,
"Kick failed: %s",
grpc_error_std_string(kick_error)
.
...
done_arg = done_arg;
storage->
next = reinterpret_cast<uintptr_t>(&cqd->completed_head) |
       static_cast<uintptr_t>(is_success);

gpr_mu_lock(cq
->mu);
cq_check_tag(cq, tag,
false); /* Used in debug builds only */
...
cq->vtable->
end_op(cq, tag, error, done, done_arg, storage, internal
);
}
...
if (!
GRPC_ERROR_IS_NONE(err)
) {
gpr_log(GPR_ERROR,
"Completion queue next failed: %s",
grpc_error_std_string(err)
.
...
gpr_mu_lock(cq
->mu);
(void)cq->poller_vtable->
kick(POLLSET_FROM_CQ(cq), nullptr);
gpr_mu_unlock(cq
->mu);
}
...
if (c->tag == tag) {
prev->
next = (prev->next & static_cast<uintptr_t>(1)) |
       (c->next & ~static_cast<uintptr_t>(1));
if (c == cqd->completed_tail) {
cqd->
completed_tail = prev;
...
return cq->vtable->
pluck(cq, tag, deadline, reserved
);
}
