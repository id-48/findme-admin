...
GPRAPI void grpc_slice_unref(grpc_slice
s);

/** Copy slice - create a new slice that contains the same data as s */
GPRAPI grpc_slice
grpc_slice_copy(grpc_slice
s);
...
grpc_slice_new(void *p, size_t len, void (*destroy)(void *));
...
grpc_slice_new_with_user_data(void *p, size_t len,
                              void (*destroy)(void *),
                              void *user_data);
...
grpc_slice_new_with_len(void *p, size_t len,
...
GPRAPI grpc_slice
grpc_slice_malloc(size_t
length);
GPRAPI grpc_slice
grpc_slice_malloc_large(size_t
length);
...
grpc_slice_from_copied_string(const char *source);
...
grpc_slice_from_copied_buffer(const char *source, size_t len);

/** Create a slice pointing to constant memory */
GPRAPI grpc_slice

grpc_slice_from_static_string(const char *source);

/** Create a slice pointing to constant memory */
GPRAPI grpc_slice

grpc_slice_from_static_buffer(const void *source, size_t len);
...
GPRAPI grpc_slice
grpc_slice_sub_no_ref(grpc_slice
s,
size_t begin, size_t
end);
...
GPRAPI grpc_slice
grpc_slice_split_tail_maybe_ref(grpc_slice
* s,
size_t split,
        grpc_slice_ref_whom
ref_whom);
...
/** Splits s into two: modifies s to be s[split:s.length], and returns a new
   ...
   Requires s initialized, split <= s.length */
GPRAPI grpc_slice
grpc_slice_split_head(grpc_slice
* s,
size_t split
);
...
grpc_empty_slice(void);

GPRAPI int grpc_slice_eq(grpc_slice
a,
grpc_slice b
);
...
/** Returns <0 if a < b, ==0 if a == b, >0 if a > b
   ...
   versions of the API. */
GPRAPI int grpc_slice_cmp(grpc_slice
a,
grpc_slice b
);
GPRAPI int grpc_slice_str_cmp(grpc_slice
a,
const char *b
);

/** return non-zero if the first blen bytes of a are equal to b */
GPRAPI int grpc_slice_buf_start_eq(grpc_slice
a,
const void *b, size_t
blen);

/** return the index of the last instance of \a c in \a s, or -1 if not found */
GPRAPI int grpc_slice_rchr(grpc_slice
s,
char c
);
GPRAPI int grpc_slice_chr(grpc_slice
s,
char c
);
...
/** return the index of the first occurrence of \a needle in \a haystack, or -1
   if it's not found */
GPRAPI int grpc_slice_slice(grpc_slice
haystack,
grpc_slice needle
);
...
GPRAPI char *grpc_slice_to_c_string(grpc_slice
s);
