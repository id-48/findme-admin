...
GRPCAPI grpc_event
grpc_completion_queue_next(grpc_completion_queue
* cq,
...
GRPCAPI grpc_event
grpc_completion_queue_pluck(grpc_completion_queue
* cq,
void *tag, gpr_timespec
deadline,
void *reserved
);
...
GRPCAPI void grpc_channel_watch_connectivity_state(
        grpc_channel * channel, grpc_connectivity_state
last_observed_state,
gpr_timespec deadline, grpc_completion_queue
* cq,
void *tag
);
...
grpc_channel_create_call(
        grpc_channel
* channel,
grpc_call *parent_call, uint32_t
propagation_mask,
grpc_completion_queue *completion_queue, grpc_slice
method,
const grpc_slice *host, gpr_timespec
deadline,
void *reserved
);
...
GRPCAPI void *grpc_call_arena_alloc(grpc_call * call, size_t
size);
...
grpc_call_start_batch(grpc_call
* call,
...
grpc_channel_create(const char *target,
...
/** Cancel an RPC.
    ...
    */
GRPCAPI grpc_call_error
grpc_call_cancel_with_status(grpc_call
* call,
...
grpc_server_request_registered_call(
        grpc_server
* server,
void *registered_method, grpc_call
** call,
gpr_timespec *deadline, grpc_metadata_array
* request_metadata,
...
grpc_server_create(const grpc_channel_args *args,
                   void *reserved);
...
GRPCAPI int grpc_server_add_http2_port(grpc_server * server,
const char *addr,
...
GRPCAPI int grpc_header_key_is_legal(grpc_slice
slice);

/** Check whether a non-binary metadata value is legal (will be accepted by
    core) */
GRPCAPI int grpc_header_nonbin_value_is_legal(grpc_slice
slice);

/** Check whether a metadata key corresponds to a binary value */
GRPCAPI int grpc_is_binary_header(grpc_slice
slice);

/** Convert grpc_call_error values to a string */
GRPCAPI const char *grpc_call_error_to_string(grpc_call_error
error);

/** Create a buffer pool */
GRPCAPI grpc_resource_quota
*

grpc_resource_quota_create(const char *trace_name);
...
GRPCAPI void grpc_resource_quota_set_max_threads(
        grpc_resource_quota * resource_quota, int
new_max_threads);
...
grpc_resource_quota_arg_vtable(void);
