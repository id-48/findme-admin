...
private
    var setupListener: (()->Void)!

            internal
    var shouldUpdateListener = true
    internal var
    configuration: FirestoreQuery < T >
    .Configuration {
        didSet{
    ...
    init<U: Decodable > (configuration
    : FirestoreQuery <T>.Configuration)
    where T
    == [U] {
        items = []
        ...
        let decodedDocuments:[U] = documents.compactMap {
...
private
    ...
    func createListener(with handler

    : @escaping (QuerySnapshot?, Error?) -> Void)
    -> () -> Void {
        return {
