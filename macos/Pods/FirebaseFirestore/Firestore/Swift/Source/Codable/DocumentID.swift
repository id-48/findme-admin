...
static func wrap(_ documentReference

: DocumentReference) throws -> Self {
...
init(from
documentReference: DocumentReference?) throws
}
...
@propertyWrapper
public
struct DocumentID<Value : DocumentIDWrappable &Codable
>:
...
private
func logIgnoredValueWarning(value
: Value) {
FirebaseLogger.
log(
        level
...
guard let
reference = decoder
        .userInfo[CodingUserInfoKey.documentRefUserInfoKey]
as? DocumentReference else {
throw FirestoreDecodingError.decodingIsNotSupported(
"""
Could not
find DocumentReference
for
user info
