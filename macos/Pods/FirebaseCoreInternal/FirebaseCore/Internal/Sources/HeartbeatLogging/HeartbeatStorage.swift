...
private
    static var cachedInstances: [String :WeakContainer < HeartbeatStorage >] = [:]
    ...
    static func getInstance(id

    : String) -> HeartbeatStorage {
        if let
        cachedInstance = cachedInstances[id] ?.object{
                return cachedInstance
        } else {
            let
            newInstance = HeartbeatStorage.makeHeartbeatStorage(id: id)
...
private
    ...
    static func makeHeartbeatStorage(id
...
#else
        let
        storage = FileStorage.makeStorage(id: id)
#endif // os(tvOS)
    ...
    func readAndWriteSync(

    using transform: (HeartbeatsBundle?) -> HeartbeatsBundle?) {
        queue.sync
        {
            let
            oldHeartbeatsBundle =
    ...
    /// - Parameter transform: A block to transform the currently stored heartbeats bundle to a new
    /// heartbeats bundle value.
    func readAndWriteAsync(

    using transform: @escaping (HeartbeatsBundle?) -> HeartbeatsBundle?) {
        queue.async
        {
            [self]
            in
    ...
    using transform: (HeartbeatsBundle?) -> HeartbeatsBundle?) throws
    -> HeartbeatsBundle? {
        let
        heartbeatsBundle:
        HeartbeatsBundle ? =
        ...
        queue.sync
        {
            ...
            let newHeartbeatsBundle = transform(oldHeartbeatsBundle)
            try
            save(newHeartbeatsBundle, to: storage)
...
private
    ...
    func load(from storage

    : Storage) throws -> HeartbeatsBundle? {
        let
        data =
        try
        storage.read()
        if data.isEmpty
        {
            ...
            try
            data.decoded(
            using: decoder) as HeartbeatsBundle
...
private
    ...
    func save(_ heartbeatsBundle

    : HeartbeatsBundle?,
    to storage: Storage
    ) throws {
        if let
        heartbeatsBundle = heartbeatsBundle{
...
extension Data{
        ...
        func decoded<T>(using decoder: JSONDecoder = .init()) throws -> T where T: Decodable {
            try
            decoder.decode(T.self, from: self)
        }
}
