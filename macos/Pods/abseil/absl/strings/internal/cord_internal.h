...
// DTRT when it sees a condition like `tag == EXTERNAL || tag >= FLAT`.
        static_assert(RING
        == BTREE + 1, "BTREE and RING not consecutive");
        static_assert(EXTERNAL
        == RING + 1, "BTREE and EXTERNAL not consecutive");
        static_assert(FLAT
        == EXTERNAL + 1, "EXTERNAL and FLAT not consecutive");
...
// We use CompressedTuple so that we can benefit from EBCO.
        ...
        struct ConstInitExternalStorage {
            ABSL_CONST_INIT static CordRepExternal
            value;
        };
...
// with all other bytes being 0.
        ...
        BigEndianByte(unsigned char value) {
        ...
        public:
            ...
            static constexpr cordz_info_t
            kNullCordzInfo = BigEndianByte(1);
