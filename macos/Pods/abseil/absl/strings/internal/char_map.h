...
namespace absl {
    ...
    namespace strings_internal {
        ...
        private:
            ...
            RangeForWord(unsigned char lo, unsigned char hi,
                         uint64_t word) {
            ...
            static constexpr uint64_t
            OpenRangeFromZeroForWord(uint64_t
            upper,
            uint64_t word
            ) {
            ...
            CharMaskForWord(unsigned char x, uint64_t word) {
...
// Mirror the char-classifying predicates in <cctype>
        ...
        UpperCharmap() { return Charmap::Range('A', 'Z'); }

        constexpr Charmap

        LowerCharmap() { return Charmap::Range('a', 'z'); }

        constexpr Charmap

        DigitCharmap() { return Charmap::Range('0', '9'); }

        constexpr Charmap

        AlphaCharmap() { return LowerCharmap() | UpperCharmap(); }

        constexpr Charmap

        AlnumCharmap() { return DigitCharmap() | AlphaCharmap(); }
        ...
        PrintCharmap() { return Charmap::Range(0x20, 0x7e); }

        constexpr Charmap

        SpaceCharmap() { return Charmap::FromString("\t\n\v\f\r "); }
        ...
        BlankCharmap() { return Charmap::FromString("\t "); }

        constexpr Charmap

        GraphCharmap() { return PrintCharmap() & ~SpaceCharmap(); }

        constexpr Charmap

        PunctCharmap() { return GraphCharmap() & ~AlnumCharmap(); }
