...
// the destructor of a CordzSampleToken object.
        ...
        ABSL_ASSERT_EXCLUSIVE_LOCK(mutex_);
...
struct List {
    ...
    SpinLock mutex;
    std::atomic<CordzInfo *> head
    ABSL_GUARDED_BY(mutex){nullptr};
};

static constexpr int kMaxStackDepth = 64;

explicit CordzInfo(CordRep
* rep,
const CordzInfo *src,
        MethodIdentifier
method);
...
void UnsafeSetCordRep(CordRep * rep)
ABSL_NO_THREAD_SAFETY_ANALYSIS;
...
inline void CordzInfo::AssertHeld()
...
inline CordRep *CordzInfo::RefCordRep() const
