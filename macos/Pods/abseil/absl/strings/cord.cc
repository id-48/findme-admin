...
// written to region and the actual size increase will be written to size.
    ...
    void Cord::InlineRep::AssignSlow(const Cord::InlineRep &src) {
        ...
        auto constexpr
        method = CordzUpdateTracker::kAssignCord;
...
// Mutators
    ...
    Cord &Cord::AssignLargeString(std::string &&src) {
        auto constexpr
        method = CordzUpdateTracker::kAssignString;
...
// we keep it here to make diffs easier.
    ...
    inline void Cord::AppendImpl(C &&src) {
        auto constexpr
        method = CordzUpdateTracker::kAppendCord;
        if (empty()) {
    ...
    CordBuffer Cord::GetAppendBufferSlowPath(size_t capacity, size_t min_capacity) {
        auto constexpr
        method = CordzUpdateTracker::kGetAppendBuffer;
        CordRep *tree = contents_.tree();
        if (tree != nullptr) {
            CordzUpdateScope scope(contents_.cordz_info(), method);
    ...
    void Cord::RemovePrefix(size_t n) {
        ...
        if (tree == nullptr) {
            ...
            auto constexpr
            method = CordzUpdateTracker::kRemovePrefix;
            CordzUpdateScope scope(contents_.cordz_info(), method);
    ...
    void Cord::RemoveSuffix(size_t n) {
        ...
        if (tree == nullptr) {
            ...
            auto constexpr
            method = CordzUpdateTracker::kRemoveSuffix;
            CordzUpdateScope scope(contents_.cordz_info(), method);
            tree = cord_internal::RemoveCrcNode(tree);
            if (n >= tree->length) {
                CordRep::Unref(tree);
...
// initializing the iterator, and returns a string_view referencing the data.
    ...
    void Cord::SetExpectedChecksum(uint32_t crc) {
        auto constexpr
        method = CordzUpdateTracker::kSetExpectedChecksum;
        if (empty()) return;
...
// Misc.
    ...
    Cord Cord::ChunkIterator::AdvanceAndReadBytes(size_t n) {
        ...
        Cord subcord;
        auto constexpr
        method = CordzUpdateTracker::kCordReader;
