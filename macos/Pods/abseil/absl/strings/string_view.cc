...
return result != nullptr ? result - ptr_ :
npos;
}
...
return result != last ? result - ptr_ :
npos;
}
...
npos;
for (
size_type i = std::min(pos, length_ - 1);; --i) {
if (ptr_[i] == c) {
return
i;
}
