...
// rotating
    ...
    noexcept {
    return
    numeric_internal::RotateLeft(x, s
    );
}
...
noexcept {
return
numeric_internal::RotateRight(x, s
);
}
...
x) noexcept {
return
numeric_internal::CountLeadingZeroes(x);
}
...
return countl_zero(static_cast
<T>(~x)
);
}
...
x) noexcept {
return
numeric_internal::CountTrailingZeroes(x);
}
...
x) noexcept {
// Avoid integer promotion to a wider type
return countr_zero(static_cast
<T>(~x)
);
}
