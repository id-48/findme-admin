...
namespace absl {
    ...
    template<class T>
    ABSL_MUST_USE_RESULT ABSL_ATTRIBUTE_ALWAYS_INLINE
    constexpr T
    RotateRight(
            T
    x,
...
<int>(
(((x
+ (x >> 4)) & 0xF0F0F0F0F0F0F0FULL) * 0x101010101010101ULL) >> 56);
#endif
}
...
? CountTrailingZeroesNonzero16(static_cast
<uint16_t>(x)
)
: (sizeof(T) <= sizeof(uint32_t)
