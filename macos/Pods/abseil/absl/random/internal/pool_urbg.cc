...
// See the allocation code in PoolAlignedAlloc().
            ...
            public:
                ...
                kState = RandenTraits::kStateBytes / sizeof(uint32_t);
                static constexpr size_t
                kCapacity =
            ...
            private:
                // Randen URBG state.
                uint32_t state_[kState]
                ABSL_GUARDED_BY(mu_);  // First to satisfy alignment.
                SpinLock mu_;
                const Randen impl_;
                size_t next_
                ABSL_GUARDED_BY(mu_);
...
// Shared pool entries.
            ...
            ABSL_CACHELINE_ALIGNED static RandenPoolEntry
            * shared_pools[kPoolSize];
