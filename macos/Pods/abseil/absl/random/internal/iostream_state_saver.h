...
#define ABSL_RANDOM_INTERNAL_IOSTREAM_HEXFLOAT 0
        ...
        ostream_state_saver<std::basic_ostream < CharT, Traits>>
        make_ostream_state_saver(
                std::basic_ostream<CharT, Traits>
        & os,  // NOLINT(runtime/references)
    ...
    typename absl::enable_if_t<!std::is_base_of<std::ios_base, T>::value,
            null_state_saver < T>>
    make_ostream_state_saver(T
    & is,  // NOLINT(runtime/references)
    std::ios_base::fmtflags flags = std::ios_base::dec
    ) {
    std::cerr << "null_state_saver";
    using result_type = null_state_saver<T>;
    return
    result_type(is, flags
    );
}
...
template<typename CharT, typename Traits>
istream_state_saver<std::basic_istream < CharT, Traits>>
