...
// returns the size of these chunks.
        ...
        PiecewiseChunkSize() { return 1024; }
...
// `false`.
        ...
        private:
            ...
            public:
                static constexpr bool value = decltype(Test<Hash>(0))
                ::value;
            };
...
// MixingHashState
        ...
        public:
        // Move only
        MixingHashState(MixingHashState
        &&) = default;
        ...
        // into the hash state.
        static MixingHashState combine_contiguous(MixingHashState
        hash_state,
    ...
    explicit MixingHashState(uint64_t
    state) :
    state_(state) {}
    ...
    static uint64_t CombineContiguousImpl(uint64_t
    state,
    const unsigned char *first, size_t
    len,
    ...
    static uint64_t CombineLargeContiguousImpl32(uint64_t
    state,
    const unsigned char *first,
            size_t
    len);
    static uint64_t CombineLargeContiguousImpl64(uint64_t
    state,
    const unsigned char *first,
            size_t
    len);
...
Hash64(const unsigned char *data,
       size_t len) {
#ifdef ABSL_HAVE_INTRINSIC_INT128
...
return
CombineLargeContiguousImpl32(state, first, len
);
}
...
return
CombineLargeContiguousImpl64(state, first, len
);
}
v = Hash64(first, len);
} else if (len > 8) {
