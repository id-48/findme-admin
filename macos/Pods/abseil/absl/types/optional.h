...
// that does not contain a value.
    struct nullopt_t {
        ...
        explicit constexpr nullopt_t(optional_internal::init_t)
...
#pragma warning(pop)
    ...
    private:
        ...
        reference() const { return this->data_; }
...
optional<typename std::decay<T>::type> (absl::forward<T>(v));
...
optional<T>(in_place_t(), absl::forward<Args>(args)
...
constexpr optional<T>
make_optional(std::initializer_list<U>
il,
Args &&... args
) {
return

optional<T>(in_place_t(), il,
...
return !y ? false : !x ? true : static_cast
<bool>(*x<
*y);
}
...
return !x ? false : !y ? true : static_cast
<bool>(*x
> *y);
}
...
return !x ? true : !y ? false : static_cast
<bool>(*x
<= *y);
}
...
return !y ? true : !x ? false : static_cast
<bool>(*x
>= *y);
}
