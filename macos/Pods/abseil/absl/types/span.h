...
template<int &... ExplicitArgumentBarrier, typename T>
Span <T> MakeSpan(T *begin, T *end)

noexcept {
return
ABSL_HARDENING_ASSERT(begin
<= end),
Span<T>(begin,
static_cast
<size_t>(end
...
MakeSpan(span_internal::GetData(c), c
...
return
Span<const T>(ptr, size
);
}

template<int &... ExplicitArgumentBarrier, typename T>
Span<const T> MakeConstSpan(T *begin, T *end)

noexcept {
return
ABSL_HARDENING_ASSERT(begin
<= end),
Span<const T>(begin, end
- begin);
}
...
MakeConstSpan(const T (&array)[N])

noexcept {
return
Span<const T>(array, N
);
}
ABSL_NAMESPACE_END
}  // namespace absl
