...
// would need to move outside its bounds.
                    bool FromWeek(int week_num, weekday week_start, year_t *year, std::tm *tm) {
                        ...
                        if (const year_t shift = cd.year() - y.year()) {
                            if (shift > 0) {
                                if (*year > std::numeric_limits<year_t>::max() - shift)
                                    return false;
                            } else {
                                if (*year < std::numeric_limits<year_t>::min() - shift)
                                    return false;
                            }
                            *year += shift;
                        }
                ...
                bool parse(const std::string &format, const std::string &input,
                    ...
                    while (data != nullptr && *fmt != '\0') {
                        ...
                        if (spec == "%p" && data != nullptr) {
                            ...
                            test_input.append(orig_data,
                                              static_cast<std::size_t>(data - orig_data));
