...
// Field normalization (without avoidable overflow).
            ...
            CONSTEXPR_F int days_per_century(int yi)

            noexcept {
            return 36524 + (yi == 0 || yi > 300);
        }

        CONSTEXPR_F int days_per_4years(int yi)
...
return
impl::n_sec(f
.y, f.m, f.d, f.hh, f.mm + n / 60, f.ss + n % 60);
}
...
return
impl::n_min(f
.y, f.m, f.d, f.hh + n / 60, 0, f.mm + n % 60, f.ss);
}
...
return
impl::n_hour(f
.y, f.m, f.d + n / 24, 0, f.hh + n % 24, f.mm, f.ss);
}
...
return
impl::n_mon(f
.y + n / 12, f.m + n % 12, f.d, 0, f.hh, f.mm, f.ss);
}
...
noexcept {
return
impl::scale_add(difference(year_tag{}, f1, f2),
12, (f1.m - f2.m));
}
...
noexcept {
return
impl::day_difference(f1
.y, f1.m, f1.d, f2.y, f2.m, f2.d);
}
...
noexcept {
return
impl::scale_add(difference(day_tag{}, f1, f2),
24, (f1.hh - f2.hh));
}
...
noexcept {
return
impl::scale_add(difference(hour_tag{}, f1, f2),
60, (f1.mm - f2.mm));
}
...
noexcept {
return
impl::scale_add(difference(minute_tag{}, f1, f2),
60, f1.ss - f2.ss);
}
...
public:
    ...
    CONSTEXPR_M year_t year() const

    noexcept { return f_.y; }
    CONSTEXPR_M int month() const

    noexcept { return f_.m; }
    CONSTEXPR_M int day() const

    noexcept { return f_.d; }
    CONSTEXPR_M int hour() const

    noexcept { return f_.hh; }
    CONSTEXPR_M int minute() const

    noexcept { return f_.mm; }
    CONSTEXPR_M int second() const

    noexcept { return f_.ss; }
    ...
    friend CONSTEXPR_F civil_time operator+(civil_time a, diff_t n)

    noexcept {
        return civil_time(step(T{}, a.f_, n));
    }

    friend CONSTEXPR_F civil_time operator+(diff_t n, civil_time a)
    ...
    friend CONSTEXPR_F diff_t operator-(civil_time lhs, civil_time rhs)
...
private:
    ...
    explicit CONSTEXPR_M civil_time(fields f)

    noexcept : f_(align(T{}, f)) {}
...
CONSTEXPR_F weekday get_weekday(const civil_second &cs)
...
CONSTEXPR_F int get_yearday(const civil_second &cs)
