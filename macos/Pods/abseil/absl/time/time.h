...
namespace absl {
    ...
    namespace time_internal {
        ...
        constexpr int64_t
        kTicksPerNanosecond = 4;
        constexpr int64_t
        kTicksPerSecond = 1000 * 1000 * 1000 * kTicksPerNanosecond;
...
// `ToInt64Microseconds()` and `ToDoubleSeconds()`.
    ...
    private:
        ...
        time_internal::GetRepHi(Duration
        d);
        friend constexpr uint32_t
        time_internal::GetRepLo(Duration
        d);
        friend constexpr Duration
        time_internal::MakeDuration(int64_t
        hi,
        uint32_t lo
        );
...
n) {
return
time_internal::FromInt64(n, std::micro{}
);
}
template<typename T, time_internal::EnableIfIntegral <T> = 0>
constexpr Duration
...
n) {
return
time_internal::FromInt64(n, std::milli{}
);
}
template<typename T, time_internal::EnableIfIntegral <T> = 0>
constexpr Duration
...
n) {
return
time_internal::FromInt64(n, std::ratio<1>{}
);
}
template<typename T, time_internal::EnableIfIntegral <T> = 0>
constexpr Duration
...
n) {
return
time_internal::FromInt64(n, std::ratio<60>{}
);
}
template<typename T, time_internal::EnableIfIntegral <T> = 0>
constexpr Duration
...
n) {
return
time_internal::FromInt64(n, std::ratio<3600>{}
);
}
...
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Nanoseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Microseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Milliseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Seconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Minutes(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION int64_t
ToInt64Hours(Duration
d);
...
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleNanoseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleMicroseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleMilliseconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleSeconds(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleMinutes(Duration
d);
ABSL_ATTRIBUTE_PURE_FUNCTION double ToDoubleHours(Duration
d);
...
FromChrono(const std::chrono::nanoseconds &d);

constexpr Duration

FromChrono(const std::chrono::microseconds &d);

constexpr Duration

FromChrono(const std::chrono::milliseconds &d);

constexpr Duration

FromChrono(const std::chrono::seconds &d);

constexpr Duration

FromChrono(const std::chrono::minutes &d);

constexpr Duration

FromChrono(const std::chrono::hours &d);
...
std::chrono::nanoseconds ToChronoNanoseconds(Duration
d);
std::chrono::microseconds ToChronoMicroseconds(Duration
d);
std::chrono::milliseconds ToChronoMilliseconds(Duration
d);
std::chrono::seconds ToChronoSeconds(Duration
d);
std::chrono::minutes ToChronoMinutes(Duration
d);
std::chrono::hours ToChronoHours(Duration
d);
...
std::string FormatDuration(Duration
d);
...
std::string AbslUnparseFlag(Duration
d);
...
std::string UnparseFlag(Duration
d);
...
private:
    ...
    time_internal::FromUnixDuration(Duration
    d);
    friend constexpr Duration
    time_internal::ToUnixDuration(Time
    t);
...
constexpr Time
FromUnixNanos(int64_t
ns);
constexpr Time
FromUnixMicros(int64_t
us);
constexpr Time
FromUnixMillis(int64_t
ms);
constexpr Time
FromUnixSeconds(int64_t
s);
constexpr Time
FromTimeT(time_t
t);
...
timespec ToTimespec(Duration
d);
timeval ToTimeval(Duration
d);
...
TimeConversion ConvertDateTime(int64_t
year,
int mon,
int day,
int hour,
...
inline Time FromDateTime(int64_t
year,
int mon,
int day,
int hour,
...
tz) {
return
ConvertDateTime(year, mon, day, hour, min, sec, tz
).
pre;
}
...
constexpr Duration
MakeNormalizedDuration(int64_t
sec,
int64_t ticks
) {
return (ticks < 0) ?
MakeDuration(sec
- 1, ticks + kTicksPerSecond)
:
...
constexpr bool IsInfiniteDuration(Duration
d) {
return
GetRepLo(d)
== ~uint32_t{
...
static_assert(0 <
N &&N
<= 1000 * 1000 * 1000, "Unsupported ratio");
// Subsecond ratios cannot overflow.
return
MakeNormalizedDuration(
        v
/ N, v %
N *kTicksPerNanosecond
* 1000 * 1000 * 1000 / N);
}
constexpr Duration
FromInt64(int64_t
...
->
decltype(int64_t{std::declval<T>()}
== 0) {
return true;
}

template<typename T>
constexpr auto IsValidRep64(char)
...
FromChrono(const std::chrono::duration<Rep, Period> &d) {
    static_assert(IsValidRep64<Rep>(0), "duration::rep is invalid");
    return FromInt64(int64_t{d.count()}, Period{});
}

template<typename Ratio>
int64_t ToInt64(Duration
d, Ratio) {
...
inline int64_t ToInt64(Duration
d, std::nano) {
return
ToInt64Nanoseconds(d);
}
inline int64_t ToInt64(Duration
d, std::micro) {
return
ToInt64Microseconds(d);
}
inline int64_t ToInt64(Duration
d, std::milli) {
return
ToInt64Milliseconds(d);
}
inline int64_t ToInt64(Duration
d, std::ratio<1>) {
return
...
template<typename T>
T ToChronoDuration(Duration
d) {
using Rep = typename T::rep;
using Period = typename T::period;
static_assert(IsValidRep64<Rep>(0),
...
constexpr bool operator<(Duration
lhs,
Duration rhs
) {
return
...
time_internal::FromUnixDuration (Nanoseconds(ns));
...
time_internal::FromUnixDuration (Microseconds(us));
...
time_internal::FromUnixDuration (Milliseconds(ms));
...
time_internal::FromUnixDuration (Seconds(s));
...
time_internal::FromUnixDuration (Seconds(t));
