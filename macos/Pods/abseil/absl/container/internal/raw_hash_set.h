...
// knows ctrl_t doesn't alias other types.
        ...
        static_assert(
        (static_cast
        <int8_t>(ctrl_t::kEmpty)
        &
        static_cast
        <int8_t>(ctrl_t::kDeleted)
        &
        static_cast
        <int8_t>(ctrl_t::kSentinel)
        & 0x80) != 0,
        ...
        static_assert(
                ctrl_t::kSentinel
        == static_cast<ctrl_t>(-1),
        "ctrl_t::kSentinel must be -1 to elide loading it from memory into SIMD "
        "registers (pcmpeqd xmm, xmm)");
        static_assert(ctrl_t::kEmpty
        == static_cast<ctrl_t>(-128),
        ...
        static_assert(ctrl_t::kDeleted
        == static_cast<ctrl_t>(-2),
        "ctrl_t::kDeleted must be -2 to make the implementation of "
        "ConvertSpecialToEmptyAndFullToDeleted efficient");

        ABSL_DLL extern const ctrl_t
        kEmptyGroup[16];
...
// Helpers for checking the state of a control byte.
        ...
        struct GroupPortableImpl {
            ...
            uint32_t CountLeadingEmptyOrDeleted() const {
                ...
                // kDeleted. We lower all other bits and count number of trailing zeros.
                constexpr
                uint64_t bits = 0x0101010101010101ULL;
...
// `Group::kWidth`-width probe window starting from any control byte.
        ...
        NumClonedBytes() { return Group::kWidth - 1; }
...
// A valid capacity is a non-zero integer `2^m - 1`.
        inline bool IsValidCapacity(size_t
        n) {
        return ((n + 1) & n) == 0 && n > 0;
    }
...
// constructed and destroyed.
    ...
    private:
        ...
        auto KeyTypeCanBeHashed(const Hash &h, const key_type &k) ->

        decltype (h(k));

        auto KeyTypeCanBeEq(const Eq &eq, const key_type &k) ->
    ...
    public:
        ...
        raw_hash_set(raw_hash_set &&that)
        ...
        slots_ (absl::exchange(that

        .slots_, nullptr)),

        size_ (absl::exchange(that

        .size_, 0)),

        capacity_ (absl::exchange(that
        ...
        std::pair<iterator, bool> emplace(Args &&... args) {
            alignas(slot_type)
            unsigned char raw[sizeof(slot_type)];
        ...
        void swap(raw_hash_set &that)

        noexcept (
        IsNoThrowSwappable<hasher>() &&

        IsNoThrowSwappable<key_equal>() &&
    ...
    private:
        ...
        friend
        struct absl::container_internal::hashtable_debug_internal::
