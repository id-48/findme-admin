...
#else
        constexpr OnDeadlockCycle
        kDeadlockDetectionDefault = OnDeadlockCycle::kAbort;
#endif

        ABSL_CONST_INIT std::atomic<OnDeadlockCycle>
        synch_deadlock_detection(
                kDeadlockDetectionDefault);
        ABSL_CONST_INIT std::atomic<bool>
        synch_check_invariants(false);
...
// Data for doing deadlock detection.
    ABSL_CONST_INIT static absl::base_internal::SpinLock
    deadlock_graph_mu(
...
// Graph used to detect deadlocks.
    ...
    *
    ...
    deadlock_graph
    ABSL_GUARDED_BY(deadlock_graph_mu)
...
// Can't be too small, as it's used for deadlock detection information.
    ...
    static struct SynchEvent {     // this is a trivial hash table for the events
        ...
        int refcount
        ABSL_GUARDED_BY(synch_event_mu);
...
// flags passed to Enqueue and LockSlow{,WithTimeout,Loop}
    ...
    static_assert(PerThreadSynch::kAlignment
    > kMuLow,
...
// acquiring and releasing a mutex in a particular mode.
    ...
    mode) {
    synch_deadlock_detection.
    store(mode, std::memory_order_release
    );
}
...
static GraphId GetGraphId(Mutex *mu)
...
static bool ExactlyOneReader(intptr_t v) {
    ...
    constexpr
    intptr_t kMuMultipleWaitersMask = kMuHigh ^ kMuOne;
