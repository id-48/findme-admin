...
static inline aes_word_t
aes_nohw_and(aes_word_t
a,
aes_word_t b
) {
return a &
...
static inline aes_word_t
aes_nohw_shift_left(aes_word_t
a,
aes_word_t i
) {
...
static inline aes_word_t
aes_nohw_shift_right(aes_word_t
a,
aes_word_t i
) {
return a >> (i * AES_NOHW_BATCH_SIZE);
...
static inline aes_word_t
aes_nohw_delta_swap(aes_word_t
a,
aes_word_t mask,
        aes_word_t
shift) {
...
aes_nohw_compact_word(uint32_t
a) {
...
static inline uint32_t
aes_nohw_uncompact_word(uint32_t
a) {
// Reverse the steps of |aes_nohw_uncompact_word|.
a = aes_nohw_delta_swap(a, 0x0000f0f0, 12);
a = aes_nohw_delta_swap(a, 0x00cc00cc, 6);
...
static inline uint32_t
aes_nohw_word_from_bytes(uint8_t
a0,
uint8_t a1,
        uint8_t
a2,
...
// down by one.
static inline aes_word_t
aes_nohw_rotate_rows_down(aes_word_t
v) {
#if defined(OPENSSL_SSE2)
...
// by two.
static inline aes_word_t
aes_nohw_rotate_rows_twice(aes_word_t
v) {
#if defined(OPENSSL_SSE2)
...
void aes_nohw_cbc_encrypt(const uint8_t *in, uint8_t *out, size_t len,
    ...
    for (;;) {
        ...
        alignas(AES_NOHW_WORD_SIZE)
        uint8_t copy[AES_NOHW_BATCH_SIZE * 16];
