...
int PKCS7_get_certificates(STACK_OF(X509)
...
int PKCS7_get_CRLs(STACK_OF(X509_CRL)
...
assert(inp
== CBS_data(&crl_data) + CBS_len(&crl_data));
...
int PKCS7_get_PEM_certificates(STACK_OF(X509)
...
int PKCS7_get_PEM_CRLs(STACK_OF(X509_CRL)
...
int PKCS7_bundle_certificates(CBB *out, const STACK_OF(X509)

*certs) {
return
pkcs7_add_signed_data(out, /*digest_algos_cb=*/NULL,
...
int PKCS7_bundle_CRLs(CBB *out, const STACK_OF(X509_CRL)
...
PKCS7 *PKCS7_sign(X509 *sign_cert, EVP_PKEY *pkey, STACK_OF(X509)
