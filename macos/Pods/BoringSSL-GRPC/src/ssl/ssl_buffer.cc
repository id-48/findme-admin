...
static_assert((SSL3_ALIGN_PAYLOAD
...
bool SSLBuffer::EnsureCap(size_t
header_len,
size_t new_cap
) {
if (new_cap > 0xffff) {
...
// may alias.
OPENSSL_memmove(new_buf
+ new_offset, buf_ + offset_, size_);
...
void SSLBuffer::DidWrite(size_t
new_size) {
if (new_size >
...
static int tls_read_buffer_extend_to(SSL * ssl, size_t
len) {
SSLBuffer *buf = &ssl->s3->read_buffer;
...
if (ret <= 0) {
ssl->s3->
rwstate = SSL_ERROR_WANT_READ;
return
ret;
}
buf->DidWrite(static_cast
<size_t>(ret)
);
}
...
int ssl_read_buffer_extend_to(SSL * ssl, size_t
len) {
...
if (!ssl->s3->read_buffer.
EnsureCap(ssl_record_prefix_len(ssl), len
)) {
return -1;
}
...
int ssl_handle_open_record(SSL * ssl, bool * out_retry, ssl_open_record_t
ret,
size_t consumed, uint8_t
alert) {
*
out_retry = false;
...
static_assert(SSL3_RT_HEADER_LENGTH
* 2 +
