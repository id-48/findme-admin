...
static constexpr
uint16_t kECHConfigVersion =
        TLSEXT_TYPE_encrypted_client_hello;

static const decltype(&EVP_hpke_aes_128_gcm)
kSupportedAEADs[] = {
...
CopyFrom (
MakeConstSpan(CBS_data(&orig), CBS_len(&orig)
...
!
CBS_get_u8(&contents, &out
->config_id) ||
!
CBS_get_u16(&contents, &out
->kem_id) ||
!
...
if (!
ssl_is_valid_ech_public_name(public_name)
) {
...
static size_t random_size(size_t
min,
size_t max
) {
assert(min<max);
...
void SSL_set_enable_ech_grease(SSL * ssl, int
enable) {
if (!ssl->config) {
return;
}
ssl->config->
ech_grease_enabled = !!enable;
}
...
if (!
ssl_is_valid_ech_config_list(span)
) {
OPENSSL_PUT_ERROR(SSL, SSL_R_INVALID_ECH_CONFIG_LIST
);
return 0;
}
return ssl->config->client_ech_config_list.
CopyFrom(span);
}
