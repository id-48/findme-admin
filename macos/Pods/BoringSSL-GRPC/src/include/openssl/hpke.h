...
OPENSSL_EXPORT const EVP_HPKE_KEM
*

EVP_hpke_x25519_hkdf_sha256(void);
...
EVP_HPKE_KEM_id(const EVP_HPKE_KEM *kem);
...
OPENSSL_EXPORT const EVP_HPKE_KDF
*

EVP_hpke_hkdf_sha256(void);

// EVP_HPKE_KDF_id returns the HPKE KDF identifier for |kdf|.
OPENSSL_EXPORT uint16_t

EVP_HPKE_KDF_id(const EVP_HPKE_KDF *kdf);
...
OPENSSL_EXPORT const EVP_HPKE_AEAD
*

EVP_hpke_aes_128_gcm(void);

OPENSSL_EXPORT const EVP_HPKE_AEAD
*

EVP_hpke_aes_256_gcm(void);

OPENSSL_EXPORT const EVP_HPKE_AEAD
*

EVP_hpke_chacha20_poly1305(void);

// EVP_HPKE_AEAD_id returns the HPKE AEAD identifier for |aead|.
OPENSSL_EXPORT uint16_t

EVP_HPKE_AEAD_id(const EVP_HPKE_AEAD *aead);

// EVP_HPKE_AEAD_aead returns the |EVP_AEAD| corresponding to |aead|.
OPENSSL_EXPORT const EVP_AEAD
*

EVP_HPKE_AEAD_aead(const EVP_HPKE_AEAD *aead);
...
// |EVP_HPKE_KEY|.
OPENSSL_EXPORT EVP_HPKE_KEY
*

EVP_HPKE_KEY_new(void);
...
EVP_HPKE_KEY_kem(const EVP_HPKE_KEY *key);
...
// This is a convenience function for callers that need a heap-allocated
// |EVP_HPKE_CTX|.
OPENSSL_EXPORT EVP_HPKE_CTX
*

EVP_HPKE_CTX_new(void);
...
OPENSSL_EXPORT int EVP_HPKE_CTX_setup_sender(
        EVP_HPKE_CTX * ctx, uint8_t * out_enc, size_t * out_enc_len, size_t
max_enc,
const EVP_HPKE_KEM *kem,
const EVP_HPKE_KDF *kdf,
const EVP_HPKE_AEAD *aead,
const uint8_t *peer_public_key, size_t
peer_public_key_len,
...
OPENSSL_EXPORT int EVP_HPKE_CTX_open(EVP_HPKE_CTX * ctx, uint8_t * out,
                                     size_t * out_len, size_t
max_out_len,
const uint8_t *in, size_t
in_len,
const uint8_t *ad, size_t
ad_len);
...
EVP_HPKE_CTX_max_overhead(const EVP_HPKE_CTX *ctx);
...
EVP_HPKE_CTX_aead(const EVP_HPKE_CTX *ctx);
...
EVP_HPKE_CTX_kdf(const EVP_HPKE_CTX *ctx);
