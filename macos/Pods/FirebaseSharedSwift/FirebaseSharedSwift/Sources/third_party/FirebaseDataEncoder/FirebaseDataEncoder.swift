...
struct NoPassthroughTypes : StructureCodingPassthroughTypeResolver {
    static func isPassthroughType<T>(_ t
...
extension DecodingError{
        ...
        internal static func _typeMismatch(at path:[CodingKey], expectation: Any.Type, reality: Any) -> DecodingError {
            ...
            return .typeMismatch(expectation,
        ...
        fileprivate static func _typeDescription(of value: Any) -> String {
            ...
            is
            NSNumber /* FIXME: If swift-corelibs-foundation isn't updated to use NSNumber, this check will be necessary: || value is Int || value is Double */ {
                    return "a number"
            } else if value
            is
            String{
...
:
_JSONStringDictionaryEncodableMarker where
Key == String, Value: Encodable {
}
...
extension Dictionary
:
_JSONStringDictionaryDecodableMarker where
...
public
    enum KeyEncodingStrategy {
        ...
        case custom((_
        codingPath: [CodingKey]) -> CodingKey)

        fileprivate static func
        _convertToSnakeCase(_
        stringKey: String) -> String {
            guard
            !stringKey.isEmpty else { return stringKey }
            ...
            var
            searchRange = stringKey.index(after: wordStart)..<stringKey.endIndex

            // Find next uppercase character
            while let
            upperCaseRange = stringKey.rangeOfCharacter(
                ...
                guard
                let
                lowerCaseRange = stringKey.rangeOfCharacter(
                        from: CharacterSet.lowercaseLetters, options: [], range: searchRange) else {
                ...
                let
                nextCharacterAfterCapital = stringKey.index(after: upperCaseRange.lowerBound)
                if lowerCaseRange.lowerBound == nextCharacterAfterCapital{
                    ...
                    let
                    beforeLowerIndex = stringKey.index(before: lowerCaseRange.lowerBound)
    ...
    open
    var passthroughTypeResolver: StructureCodingPassthroughTypeResolver.Type = NoPassthroughTypes.self
    ...
    fileprivate struct _Options {
        ...
        keyEncodingStrategy: KeyEncodingStrategy
                let
        passthroughTypeResolver: StructureCodingPassthroughTypeResolver.Type
                let
...
public
    ...
    open func
    encode<T : Encodable > (_ value
    : T) throws -> Any {
        let
        encoder = __JSONEncoder(options: self.options)

        guard
        let
        topLevel =
        try
        encoder.box_(value) else {
...
class __JSONEncoder : Encoder {
    ...
    storage: _JSONEncodingStorage
            ...
            fileprivate
    let options: FirebaseDataEncoder._Options
...
public
    ...
    fileprivate init(options

    : FirebaseDataEncoder._Options,
    codingPath:
    [CodingKey] = []) {
...
public
    ...
    : Key.Type) -> KeyedEncodingContainer <Key> {
        ...
        if self.canEncodeNewValue
        {
            ...
            guard
            let
            container = self.storage.containers.last
        ...
        let
        container = _JSONKeyedEncodingContainer<Key>(
...
private(set)
    ...
    count: Int{
            ...
            fileprivate
    mutating func
    pushKeyedContainer() -> NSMutableDictionary {
...
fileprivate init(referencing encoder

: __JSONEncoder, codingPath: [CodingKey],
wrapping container
: NSMutableDictionary) {
self.
encoder = encoder
...
return
_JSONKey(stringValue
: newKeyString, intValue: key.intValue)
case .
custom(let
converter):
return
converter(codingPath
+ [key])
}
}
...
if T.
self is
StructureCodingUncodedUnkeyed.Type {
return }
self.encoder.codingPath.
append(key)
defer {
...
public
mutating func
nestedContainer<NestedKey>(keyedBy
keyType: NestedKey.Type,
forKey key
: Key) -> KeyedEncodingContainer <NestedKey> {
...
let container = _JSONKeyedEncodingContainer < NestedKey > (referencing
: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
return
...
key: Key) -> UnkeyedEncodingContainer {
let containerKey = _converted(key).stringValue
let array
: NSMutableArray
if
...
return
_JSONUnkeyedEncodingContainer(referencing
: self.encoder, codingPath: self.codingPath, wrapping: array)
}

public
mutating func
superEncoder() -> Encoder {
return
__JSONReferencingEncoder(referencing
: self.encoder, key: _JSONKey.super, convertedKey:
_converted(_JSONKey
.super), wrapping: self.container)
}
...
return
__JSONReferencingEncoder(referencing
: self.encoder, key: key, convertedKey:
_converted(key), wrapping
: self.container)
}
}
...
public
    var count: Int{
            ...
            fileprivate
    init(referencing
    encoder: __JSONEncoder, codingPath:
    [CodingKey],
...
public
    ...
    throws { self.container.add(NSNull()) }
public
    mutating func
    encode(_
    value: Bool
    )   throws { self.container.add(self.encoder.box(value)) }
public
    mutating func
    encode(_
    value: Int
...
public
    ...
    keyType: NestedKey.Type
        ...
        let
        container = _JSONKeyedEncodingContainer < NestedKey >
...
public
    mutating func
    superEncoder() -> Encoder {
        return __JSONReferencingEncoder(
                referencing: self.encoder, at: self.container.count, wrapping: self.container)
    }
}

        extension
__JSONEncoder : SingleValueEncodingContainer {
...
extension __JSONEncoder{
        ...
        fileprivate func box(_ float: Float) throws -> NSObject {
            guard
            !float.isInfinite && !float.isNaN else {
                guard
                case let.convertToString(positiveInfinity:
                    posInfString,
        ...
        fileprivate func box(_ double: Double) throws -> NSObject {
            guard
            !double.isInfinite && !double.isNaN else {
                guard
                case let.convertToString(positiveInfinity:
                    posInfString,
                            negativeInfinity: negInfString,
        ...
        fileprivate func box_(_ value: Encodable) throws -> NSObject? {
            ...
            object = value
            as ? NSObject, self.options.passthroughTypeResolver.isPassthroughType(value) {
                    return object
            }
...
private
    ...
    : __JSONEncoder,
    key: CodingKey, convertedKey: __shared CodingKey, wrapping
    dictionary: NSMutableDictionary
    ) {
        self.encoder = encoder
...
public enum DateDecodingStrategy {
        ...
        case custom((_
        decoder: Swift.Decoder) throws -> Date)
    }
...
public enum DataDecodingStrategy {
        ...
        case custom((_
        decoder: Swift.Decoder) throws -> Data)
    }
...
public
    enum KeyDecodingStrategy {
        ...
        stringKey: String) -> String {
            ...
            guard
            let
            firstNonUnderscore = stringKey.firstIndex(where: { $0 != "_" }) else {
            ...
            var
            lastNonUnderscore = stringKey.index(before: stringKey.endIndex)
            ...
            let
            keyRange = firstNonUnderscore...lastNonUnderscore
            let leadingUnderscoreRange = stringKey.startIndex..< firstNonUnderscore
            let
            trailingUnderscoreRange = stringKey.index(after: lastNonUnderscore)..<stringKey.endIndex

            let
            components = stringKey[keyRange].split(separator: "_")
            let
            joinedString :
            String
...
public
    ...
    open func
    decode<T : Decodable > (_ type
    : T.Type,
    from structure: Any
    ) throws -> T {
        let
        decoder = __JSONDecoder(referencing: structure, options: self.options)
        guard
        let
        value =
        try
        decoder.unbox(structure, as: type) else {
            throw Swift.DecodingError.valueNotFound(type, Swift.DecodingError.Context(
...
class __JSONDecoder : Decoder {
    ...
    fileprivate var
    storage: _JSONDecodingStorage

            /// Options set on the top-level decoder.
            fileprivate
    let options: FirebaseDataDecoder._Options
...
public
    ...
    fileprivate init(referencing container

    : Any,
    at codingPath:
    [CodingKey] = [],
    options: FirebaseDataDecoder._Options
    ) {
...
public
    ...
    func container<Key>(keyedBy type

    : Key.Type) throws -> KeyedDecodingContainer <Key> {
        guard
        !(self.storage.topContainer
        is
        NSNull) else {
        ...
        Any]
        if let
        rcValue = self.storage.topContainer
        as ? FirebaseRemoteConfigValueDecoding{
                guard let top = rcValue.dictionaryValue() else {
                    throw DecodingError._typeMismatch(at: self.codingPath, expectation: [String :
                    Any].self,
                            reality: rcValue)
                }
                topContainer = top
            ...
            guard
            let
            top = self.storage.topContainer
            as ? [String :
            Any] else {
                throw DecodingError._typeMismatch(at: self.codingPath, expectation: [String :
                Any].self, reality: self.storage.topContainer)
            }
            topContainer = top
        }

        let
        container = _JSONKeyedDecodingContainer<Key>(referencing: self, wrapping: topContainer)
...
public
    ...
    throws -> UnkeyedDecodingContainer {
        ...
        as ? FirebaseRemoteConfigValueDecoding{
                guard let arrayValue = rcValue.arrayValue() else {
                    throw DecodingError._typeMismatch(
                            at: self.codingPath, expectation: [Any].self, reality: rcValue)
                }
        ...
        guard
        let
        topContainer = self.storage.topContainer
        as ? [Any] else {
            throw DecodingError._typeMismatch(
...
container = Dictionary(container.map {
    key, value
    in(FirebaseDataDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
}
...
container = Dictionary(container.map {
    key, value
    in(converter(decoder.codingPath + [_JSONKey(
            stringValue: key, intValue: nil)]).stringValue, value)
}
...
public
func decodeNil(forKey
key: Key) throws -> Bool {
guard let
entry = self.container[key.stringValue]
else {
throw DecodingError.
keyNotFound(key, DecodingError
.
Context(codingPath
: self.decoder.codingPath, debugDescription: "No value associated with key \(_errorDescription(of: key))."))
}
...
: Bool.self) else {
throw DecodingError.
valueNotFound(type, DecodingError
.
Context(codingPath
: self.decoder.codingPath, debugDescription: "Expected \(type) value but found null instead."))
}
...
public
func nestedContainer<NestedKey>(keyedBy
type: NestedKey.Type,
forKey key
: Key) throws -> KeyedDecodingContainer <NestedKey> {
self.decoder.codingPath.
append(key)
defer {
self.decoder.codingPath.
...
guard let
value = self.container[key.stringValue]
else {
throw DecodingError.
keyNotFound(key,
        DecodingError
...
throw DecodingError.
_typeMismatch(at
: self.codingPath, expectation: [String : Any].self, reality: value)
}

let container = _JSONKeyedDecodingContainer < NestedKey > (referencing
: self.decoder, wrapping: dictionary)
return
KeyedDecodingContainer(container)
}

public
func nestedUnkeyedContainer(forKey
key: Key) throws -> UnkeyedDecodingContainer {
self.decoder.codingPath.
append(key)
defer {
self.decoder.codingPath.
...
throw DecodingError.
_typeMismatch(at
: self.codingPath, expectation: [Any].self, reality: value)
}

return
_JSONUnkeyedDecodingContainer(referencing
: self.decoder, wrapping: array)
}

private
func _superDecoder(forKey
key:
__owned CodingKey
) throws -> Decoder {
self.decoder.codingPath.
...
let value
:
Any = self.container[key.stringValue] ?
?

NSNull()

return
__JSONDecoder(referencing
: value, at: self.decoder.codingPath, options: self.decoder.options)
}
...
throws -> Decoder {
return try
_superDecoder(forKey
: _JSONKey.super)
}

public
func superDecoder(forKey
key: Key) throws -> Decoder {
return try
_superDecoder(forKey
: key)
}
...
public
    var currentIndex: Int
            ...
            fileprivate
    init(referencing
    decoder: __JSONDecoder, wrapping
    container:
    [Any]) {
...
public
    ...
    throws -> Bool {
        ...
        !self.isAtEnd else {
            ...
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Unkeyed container is at end."))
        }

        if self.container[self.currentIndex]
        is
        NSNull{
                self.currentIndex += 1
                return true
...
public
    ...
    type: Bool.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Bool.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
...
public
    ...
    type: Int.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Int.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Int8.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Int8.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Int16.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Int16.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Int32.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Int32.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Int64.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Int64.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: UInt.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: UInt.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: UInt8.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: UInt8.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: UInt16.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: UInt16.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: UInt32.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: UInt32.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: UInt64.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: UInt64.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Float.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Float.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: Double.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: Double.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: String.Type
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: String.self) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    : T.Type) throws -> T {
        ...
        decoded =
        try
        self.decoder.unbox(self.container[self.currentIndex], as: type) else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath:
            self.decoder.codingPath + [_JSONKey(index: self.currentIndex)], debugDescription:
            "Expected \(type) but found null instead."))
        }
...
public
    ...
    type: NestedKey.Type
        ...
        Any] else {
            throw DecodingError._typeMismatch(at: self.codingPath, expectation: [String :
            Any].self, reality: value)
        }

        self.currentIndex += 1
        let
        container = _JSONKeyedDecodingContainer < NestedKey >
...
public
    ...
    throws -> Decoder {
        ...
        let value = self.container[self.currentIndex]
        self.currentIndex += 1
        return __JSONDecoder(
                referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
}

        extension
__JSONDecoder : SingleValueDecodingContainer {
// MARK: SingleValueDecodingContainer Methods

private
func expectNonNull<T>(_
type: T.Type) throws {
guard !self.
...
.
Context(codingPath
: self.codingPath, debugDescription: "Expected \(type) but found null value instead."))
}
}
...
public
func decode(_
type: Bool.Type) throws -> Bool {
try
expectNonNull(Bool
.self)
return try self.
unbox(self
.storage.topContainer, as: Bool.self)!
}

public
func decode(_
type: Int.Type) throws -> Int {
try
expectNonNull(Int
.self)
return try self.
unbox(self
.storage.topContainer, as: Int.self)!
}

public
func decode(_
type: Int8.Type) throws -> Int8 {
try
expectNonNull(Int8
.self)
return try self.
unbox(self
.storage.topContainer, as: Int8.self)!
}

public
func decode(_
type: Int16.Type) throws -> Int16 {
try
expectNonNull(Int16
.self)
return try self.
unbox(self
.storage.topContainer, as: Int16.self)!
}

public
func decode(_
type: Int32.Type) throws -> Int32 {
try
expectNonNull(Int32
.self)
return try self.
unbox(self
.storage.topContainer, as: Int32.self)!
}

public
func decode(_
type: Int64.Type) throws -> Int64 {
try
expectNonNull(Int64
.self)
return try self.
unbox(self
.storage.topContainer, as: Int64.self)!
}

public
func decode(_
type: UInt.Type) throws -> UInt {
try
expectNonNull(UInt
.self)
return try self.
unbox(self
.storage.topContainer, as: UInt.self)!
}

public
func decode(_
type: UInt8.Type) throws -> UInt8 {
try
expectNonNull(UInt8
.self)
return try self.
unbox(self
.storage.topContainer, as: UInt8.self)!
}

public
func decode(_
type: UInt16.Type) throws -> UInt16 {
try
expectNonNull(UInt16
.self)
return try self.
unbox(self
.storage.topContainer, as: UInt16.self)!
}

public
func decode(_
type: UInt32.Type) throws -> UInt32 {
try
expectNonNull(UInt32
.self)
return try self.
unbox(self
.storage.topContainer, as: UInt32.self)!
}

public
func decode(_
type: UInt64.Type) throws -> UInt64 {
try
expectNonNull(UInt64
.self)
return try self.
unbox(self
.storage.topContainer, as: UInt64.self)!
}

public
func decode(_
type: Float.Type) throws -> Float {
try
expectNonNull(Float
.self)
return try self.
unbox(self
.storage.topContainer, as: Float.self)!
}

public
func decode(_
type: Double.Type) throws -> Double {
try
expectNonNull(Double
.self)
return try self.
unbox(self
.storage.topContainer, as: Double.self)!
}

public
func decode(_
type: String.Type) throws -> String {
try
expectNonNull(String
.self)
return try self.
unbox(self
.storage.topContainer, as: String.self)!
}
...
extension __JSONDecoder{
       ...
       */
        ...
        fileprivate func getNumber(_ value: Any, as type: Any.Type) throws -> NSNumber {
            ...
            let
            number = val
            as ? NSNumber, number != = kCFBooleanTrue, number != = kCFBooleanFalse else {
                throw DecodingError._typeMismatch(
                        at: self.codingPath, expectation: type, reality: val)
            }
            return number
        }
        ...
        fileprivate func unbox(_ value: Any, as type: Int.Type) throws -> Int? {
            ...
            number =
            try
            getNumber(value, as: type)
            let
            int = number.intValue
            guard
            NSNumber(value: int) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: Int8.Type) throws -> Int8? {
            ...
            guard
            NSNumber(value: int8) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: Int16.Type) throws -> Int16? {
            ...
            guard
            NSNumber(value: int16) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: Int32.Type) throws -> Int32? {
            ...
            guard
            NSNumber(value: int32) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: Int64.Type) throws -> Int64? {
            ...
            guard
            NSNumber(value: int64) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: UInt.Type) throws -> UInt? {
            ...
            guard
            NSNumber(value: uint) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: UInt8.Type) throws -> UInt8? {
            ...
            guard
            NSNumber(value: uint8) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: UInt16.Type) throws -> UInt16? {
            ...
            guard
            NSNumber(value: uint16) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: UInt32.Type) throws -> UInt32? {
            ...
            guard
            NSNumber(value: uint32) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: UInt64.Type) throws -> UInt64? {
            ...
            guard
            NSNumber(value: uint64) == number else {
                throw DecodingError.dataCorrupted(DecodingError.Context(
                        codingPath: self.codingPath, debugDescription: "Parsed JSON number <\(number)> does not fit in \(type)."))
            }
        ...
        fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
            ...
            if let
            number = val
            as ? NSNumber, number != = kCFBooleanTrue, number != = kCFBooleanFalse{
       ...
       */
            ...
            string = val
            as ? String,
            case .convertFromString(let
            posInfString, let
            negInfString, let
            nanString) = self.options.nonConformingFloatDecodingStrategy
            {
       ...
       */
        ...
        fileprivate func unbox(_ value: Any, as type: Date.Type) throws -> Date? {
            ...
            switch self.options.dateDecodingStrategy
            {
                ...
                double =
                try
                self.unbox(value, as: Double.self)!
                ...
                10.0, *) {
                    ...
                    string =
                    try
                    self.unbox(value, as: String.self)!
                            guard
                    let
                    date = _iso8601Formatter.date(from: string) else {
                        throw DecodingError.dataCorrupted(DecodingError.Context(
                                codingPath: self.codingPath, debugDescription: "Expected date string to be ISO8601-formatted."))
                    }
                ...
                self.unbox(value, as: String.self)!
                        guard
                let
                date = formatter.date(from: string) else {
                    throw DecodingError.dataCorrupted(DecodingError.Context(
                            codingPath: self.codingPath, debugDescription: "Date string does not match format expected by formatter."))
                }
        ...
        fileprivate func unbox(_ value: Any, as type: Data.Type) throws -> Data? {
            ...
            switch self.options.dataDecodingStrategy
            {
                ...
                guard
                let
                data = Data(base64Encoded: string) else {
                    throw DecodingError.dataCorrupted(DecodingError.Context(
                ...
                string = value
                as ? String, let data = Data(base64Encoded: string) {
        ...
        fileprivate func unbox<T>(_ value: Any, as type: _JSONStringDictionaryDecodableMarker.Type) throws -> T? {
            ...
            as ? FirebaseRemoteConfigValueDecoding{
                    guard let dictionaryValue = rcValue.dictionaryValue() else {
                        throw DecodingError._typeMismatch(
                                at: self.codingPath, expectation: type, reality: rcValue)
                    }
        ...
        fileprivate func unbox_(_ value: Any, as _type: Decodable.Type) throws -> Any? {
            if _type == Date.self || _type == NSDate.self
            {
                return
                try
                self.unbox(value, as: Date.self)
            } else if _type == Data.self || _type == NSData.self
            {
                return
                try
                self.unbox(value, as: Data.self)
            } else if _type == URL.self || _type == NSURL.self
            {
                ...
                urlString =
                try
                self.unbox(value, as: String.self) else {
                    return nil
                }
            ...
            } else if _type == Decimal.self || _type == NSDecimalNumber.self
            {
                return
                try
                self.unbox(value, as: Decimal.self)
            } else if let
            stringKeyedDictType = _type
            as ? _JSONStringDictionaryDecodableMarker.Type{
...
@
available(macOS
10.12, iOS 10.0, watchOS 3.0, tvOS 10.0, *)
fileprivate var
_iso8601Formatter:
...
extension EncodingError{
        ...
        fileprivate static func _invalidFloatingPointValue<T : FloatingPoint>(_ value: T, at codingPath:[CodingKey]) -> EncodingError {
            ...
            return .invalidValue(value, EncodingError.Context(
