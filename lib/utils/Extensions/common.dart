...
void afterBuildCreated(Function()? onCreated) {
  makeNullable(SchedulerBinding.instance)!.addPostFrameCallback((_) =>
      onCreated?.call());
}

Route<T> buildPageRoute<T>(Widget child, PageRouteAnimation? pageRouteAnimation,
    Duration? duration) {
...
Widget appButton(BuildContext context,
    {String? title, Function? onCall, Color? color, Color? textColor}) {
  return InkWell(
    onTap: () {
      onCall!();
    },
    child: Container(
      ...
      decoration: BoxDecoration(
          borderRadius: radius(defaultRadius), color: color ?? primaryColor),
      child: Text(
          title!, style: boldTextStyle(color: textColor ?? Colors.white)),
    ),
  );
}
...
// toast(String? value, {ToastGravity? gravity, length = Toast.LENGTH_LONG, Color? bgColor, Color? textColor}) {
